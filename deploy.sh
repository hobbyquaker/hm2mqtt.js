...
# hm2mqtt@<name> services there. Same script as in lgtv2mqtt / lgsb2mqtt.
#
# Usage:
...
#
# Optional env vars:
#   REMOTE_DIR   (default: /usr/local/lib/node_modules/hm2mqtt)
#   REMOTE_TMP   (default: /tmp)
#   SERVICE      systemd unit(s) to restart after deploying; default: every active
#                hm2mqtt@<name> instance plus a plain hm2mqtt unit if present
...
# mqtt-interfaces-core, homematic-rega, homematic-xmlrpc) are packed and shipped too; registry versions need nothing extra.
#
# The deploy does not create a service; install one once on the target with
#   sudo hm2mqtt --install -n hm -a homematic-ccu3 -u mqtt://broker
...
if [[ -n "$SERVICE" ]]; then
  ...
  echo "No $PKG service found to restart (install one with: sudo $PKG --install -n hm -a homematic-ccu3 -u mqtt://broker)"
fi

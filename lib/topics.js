 ...
 * publishing exactly where it did. A topic template the user changed wins over the deprecated
 * option; either way the migration explains itself in `config.$warnings`.
 * @param {object} config
 * @returns {object} the same config, with the topic templates filled in
 */
export function applyItemTemplates(config) {
    const pairs = [
        ['itemTemplate', 'topicStatus', DEFAULT_TOPIC_STATUS, 'topicSet', DEFAULT_TOPIC_SET],
        [
            'sysvarItemTemplate',
            'topicSysvarStatus',
    ...
    for (const [item, status, statusDefault, set, setDefault] of pairs) {
        if (!config[item]) {
            continue;
        }
        ...
        const changed = [
            ...
            .filter(([key, fallback]) => config[key] !== fallback)
            .map(([key]) => '--' + kebab(key));
        if (changed.length > 0) {
            config.$warnings.push(
                `--${kebab(item)} is deprecated and ignored here because ${changed.join(' and ')} ${changed.length > 1 ? 'are' : 'is'} set`,
            );
            continue;
        }
        config.$warnings.push(`--${kebab(item)} is deprecated - use --${kebab(status)} and --${kebab(set)}`);

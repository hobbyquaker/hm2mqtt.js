...
/** a failed init is retried after 1, 2, 4, 8 s, then every INIT_RETRY_MS (the cap; task 21) */
const INIT_BACKOFF_MS = [1000, 2000, 4000, 8000];
export const INIT_RETRY_MS = 15000;
...
/**
 * Whether an error only says that nothing listens on the port yet - an interface process that is
 * still starting (openccu-lite starts the addon before rfd and hmipserver), or one that is
 * restarting. Such a wait is normal at boot and logged as info, not as a warning.
 * @param {Error|string} err
 * @returns {boolean}
 */
export function notListening(err) {
...
/**
 * The delay before the next init attempt.
 * @param {number} failures failed attempts in a row (1 after the first failure)
 * @returns {number} ms
 */
export function initRetryDelay(failures) {
    return failures >= 1 && failures <= INIT_BACKOFF_MS.length ? INIT_BACKOFF_MS[failures - 1] : INIT_RETRY_MS;
}
...
export class RpcServers {
     ...
     * @param {string} [o.ownPrefix] the start of this instance's init ids: a call for one of them that
     *   is not registered (yet) is expected - an interface process that re-announces itself to a
     *   subscription of the last run before this one subscribed again - and logged as debug
     */
    constructor({listenAddress, initAddress, xmlrpcPort, binrpcPort, log, libs = {}, ownPrefix}) {
    ...
    connectionFor(params) {
        ...
        if (!connection) {
            const own = this.ownPrefix && typeof initId === 'string' && initId.startsWith(this.ownPrefix);
            (own ? this.log.debug : this.log.warn)('rpc < call for unknown init id', String(initId));
        }
        return connection;
    }
...
export class RpcConnection extends EventEmitter {
     ...
     * @param {number} [o.callTimeout] ms a method call may take (default 30000)
    ...
    constructor(o) {
        ...
        this.callTimeout = o.callTimeout || CALL_TIMEOUT_MS;
    ...
    /** Closes the client (binrpc: its socket and reconnect timer); the next call creates a new one. */
    dropClient() {
        if (this.protocol !== 'binrpc' || !this.client) {
            return;
        }
    ...
    methodCall(method, params = []) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const timer = setTimeout(() => {
                ...
                reject(new Error(`${method} timed out after ${this.callTimeout / 1000} s`));
            }, this.callTimeout);
    ...
    /** Connects: init subscription (retried with backoff from 2 s while it fails), ping supervision. */
    async start() {
        this.stopped = false;
        if (!this.wantInit) {
            try {
    ...
    async init() {
        ...
        try {
            ...
            if (this.initFailures > 0) {
                this.log.info('rpc', this.name, 'init succeeded after', this.initFailures + 1, 'attempts');
            }
            this.initFailures = 0;
            this.lastInitError = null;
            this.setConnected(true);
            ...
            this.initFailures += 1;
            const delay = initRetryDelay(this.initFailures);
            // a binrpc client whose socket is gone waits for its own reconnect timer (2.5 s doubling
            // up to 30 s) and fails every call meanwhile; a fresh one connects at the next attempt
            this.dropClient();
            const retry = `retrying in ${delay / 1000} s (then up to every ${INIT_RETRY_MS / 1000} s)`;
            if (msg === this.lastInitError) {
                this.log.debug('rpc', this.name, 'init failed:', msg, `- next attempt in ${delay / 1000} s`);
            } else if (!this.everInitialized && notListening(err)) {
                // the start before the interface process (openccu-lite's early start): a wait, not a fault
                this.log.info('rpc', this.name, `nothing is listening on ${this.host}:${this.port} yet - ${retry}`);
                this.lastInitError = msg;
            } else {
                this.log.warn('rpc', this.name, 'init failed:', msg, `- ${retry}`);
                this.lastInitError = msg;
            }
            this.setConnected(false);
            this.initTimer = this.timers.setTimeout(() => this.init(), delay);
            return;
        }
        if (this.wantPing) {

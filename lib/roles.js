/**
 * Semantic roles of channels and the Home Assistant facts of datapoints — derived from the
 * paramset descriptions (the CONTROL hint eQ-3 puts on every parameter), channel types as
 * fallback (ROADMAP H-21). Pure functions and tables, no I/O.
 */

/** CONTROL hint of a channel's primary datapoint → role; checked in this order */
export const ROLE_BY_CONTROL = [
    ['LOCK.STATE', 'lock'],
...
/** channel types whose role does not come from a CONTROL hint (older HM devices, maintenance) */
 ...
 * @returns {string | null}
 */
export function channelRole(channelType, description) {
...
export function isFraction(description) {
...
/**
 * Compiles `--ignore` globs on `<iface>.<channel>.<datapoint>` (`*` any characters, `?` one).
 * @param {string | string[]} patterns comma separated or array
 * @returns {(iface: string, channel: string, datapoint: string) => boolean}
 */
export function compileIgnore(patterns) {
    ...
    return (iface, channel, datapoint) => {
        const name = `${iface}.${channel}.${datapoint}`;

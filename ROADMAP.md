...
| H-43 | **Logging goes to syslog**, `logger -t hm2mqtt -p daemon.info`, not to a file of our own: no rotation to maintain, no disk to fill on a CCU3, and a CCU that forwards syslog (this one does) carries hm2mqtt's log with it. `log.cgi` reads `/var/log/messages` and `messages.0` filtered by tag, the way RedMatic's log view does.                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| OQ-63 | **The CCU3 kernel is 4.14.34**; Node's `BUILDING.md` states kernel ≥ 4.18 for armv7. That figure belongs to the official _glibc_ builds (glibc's own kernel floor), so it does not transfer to Alpine's musl binary — musl supports far older kernels and libuv probes the modern syscalls (`io_uring`, `epoll_pwait2`, `close_range`) with fallbacks. Residual risk: V8 or libuv calling something post-4.14 unconditionally. | Not answerable from documentation — run the packaged binary on a CCU3 once (`node -e`, then `npm test`). Ladder if it fails: Node 22 → 20 for armv7l only. This is the single unknown that gates the armv7l package (OQ-62).                                                        |

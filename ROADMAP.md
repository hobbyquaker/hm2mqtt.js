...
- Node still _supports building_ for armv7: `BUILDING.md` lists GNU/Linux armv7 as **Experimental,
  "downgraded as of Node.js 24"** — dropped from the release binaries, not from the source tree. The
  proof that 24 works on 32-bit ARM is downstream: **Debian sid ships `nodejs 24.19.0` for armhf**.
...
| H-39 | **We build the armv7l runtime ourselves** so the official CCU3 firmware is supported (§15.2), **as a musl fully-static binary**: no glibc floor, no `libstdc++` floor, one artefact for official firmware and OpenCCU alike, and the CCU's ancient userland stops being a variable. Cross-compiled on x86_64 (musl-cross-make toolchain, gcc ≥ 12.2 as Node 24 requires), never QEMU. Fallback if the static build misbehaves: a glibc-2.27 cross-build with `-static-libstdc++ -static-libgcc` (OQ-61). aarch64/x86_64 keep the stock tarball.                                                                                                                  |
| H-40 | **The runtime build lives in its own repo**, not in hm2mqtt: revive **`hobbyquaker/ccu-addon-nodejs`** (dead since 2018, exactly this purpose, the name is already the right one) as the place that builds and releases Node for CCU hardware; assets `node-v<ver>-linux-armv7l-musl-static.tar.xz` + `.sha256` per release. Reasons: its cadence is per Node version, not per hm2mqtt release; hm2mqtt's tag space stays hm2mqtt versions (H-41); other CCU addon authors get the same artefact; and hm2mqtt's CI stays a two-minute job. The addon build downloads it by pinned URL + sha256. This does not reopen H-36 — the addon itself stays in this repo. |
...
2. Runtime: `curl https://nodejs.org/dist/v$NODE/$NAME.tar.xz` for aarch64/x86_64, and for armv7l our own
   `node-v$NODE-linux-armv7l-musl-static.tar.xz` from the runtime repo's release (H-39, H-40) — same unpack, same strip per H-30.
...
The armv7l **runtime** is not part of this: it is built by a separate manual workflow
(`workflow_dispatch`, input = Node version) that produces the glibc-2.27 tarball of H-39 and
publishes it under a `runtime-node-<version>` tag. It runs when the Node pin moves, a few times a
year, and its output is cached by URL in the addon build.
...
| 1b  | Revive `ccu-addon-nodejs` (H-40): musl-cross-make toolchain in a `workflow_dispatch` job, Node 24 cross-compiled fully static for armv7l, smoke test on a real CCU3 (kernel 4.14, OQ-63), release asset + sha256. Blocks the armv7l package, blocks nothing else.  | 1–2 d  |
...
| OQ-61 | **musl-static or glibc-2.27** for the armv7l runtime (H-39)? Static musl removes both floors (glibc 2.28, GLIBCXX 3.4.26) and yields one binary for every CCU; a glibc-2.27 cross-build is a conventional dynamic binary but re-couples us to eQ-3's 2019 userland. Cost is similar: both need a cross toolchain with gcc ≥ 12.2, ~1–2 h per Node version in CI. | musl-static first (portability, and it also survives whatever eQ-3 ships next); keep the glibc-2.27 route documented as the fallback. Decide by building both once and running them on the CCU3.                 |
| OQ-63 | Node's `BUILDING.md` asks for **kernel ≥ 4.18** on armv7; the CCU3 runs **4.14.34**. Is that a real runtime dependency (a syscall V8/libuv needs) or just the build baseline?                                                                                                                                                                                    | Answer empirically with the first self-built binary: run the hm2mqtt test suite on the CCU3. If a syscall is genuinely missing, the fallback ladder is Node 22 → 20 for armv7l only, at the cost of a mixed pin. |
| OQ-62 | **Who tests the addon on real hardware** — the `-beta` suffix (H-40) only comes off with one confirmed install on a CCU3 with official firmware and one on OpenCCU. The user's own CCU3 is off limits for now.                                                                                                                                                   | Ask in the RedMatic/homematic forum when the first package builds; until then ship `-beta` and say so in the release notes.                                                                                      |

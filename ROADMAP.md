...
**Status 2026-08-28: 3.1.0 released (`--payload`); HA discovery (§13, H-21…H-27) and B-4 implemented in 3.2.0 on master — validated structurally against the real device table (64 device types, 254 VALUES descriptions from this CCU as test fixtures), not against a live Home Assistant (no Docker on this Mac). Discovery is on by default (OQ-53 → on, per D-5, generic entities disabled by default). `hm2mqtt@hm3` still runs 3.0.0; a redeploy would publish the retained discovery topics on the broker — set `HM2MQTT_HA_DISCOVERY=false` in `/etc/hm2mqtt/hm3.env` first if that is unwanted.**
...
| ID  | Item                                                                                                                                                                                                                                                                                                           | Effort | Slot         |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------------ |
| B-1 | **done 2026-08-27** — **Description fetching like CCU-Jack**: `VALUES` eagerly, `MASTER` lazily on first `paramset` use, `LINK`/`SERVICE` never (battery devices do not answer — the source of our `getParamsetDescription` timeouts), 50 ms spacing instead of 200 ms. 537 HmIP descriptions: ~2 min → ~20 s. | 0.5 h  | 3.0.0        |
| B-2 | **done 2026-08-27** — **Retained-topic cleaner for the cutover** (loom keeps a one-shot topic migrator): `scripts/clean-retained.js <url> <pattern>…` — the flow left 2291 nameless `hm/status//X` topics and years of stale items. Needed by §9.                                                              | 1 h    | 3.0.0        |
| B-3 | **done 2026-08-27** — **Periodic name re-sync**: `--rega-names-interval 3600` (`getChannels` costs 250 ms) so renames, rooms and functions propagate without a restart; `set/rega/sync` stays for immediate.                                                                                                   | 0.5 h  | 3.0.0        |
| B-4 | **done 2026-08-28 (3.2.0)** — **Datapoint ignore filter**: `--ignore '*.*.RSSI_*,HmIP-RF.*.*_STATUS'` (globs on `iface.channel.datapoint`) — a light version of loom's visibility model; cuts HmIP chatter and the HA entity flood (§13, OQ-53).                                                               | 1 h    | 3.0.x        |
...
**Status 2026-08-31: steps 1–8 done.** The package builds for all three architectures
(`addon/build.sh`), the runtime is repackaged from Alpine without a container in ~18 s, the CGIs and
the Svelte UI are in, local mode ships, and the release workflow attaches the packages with an
installation table in the notes. Verified on the CCU3 short of an actual install (OQ-62): the
runtime runs, `child_process` works through the patched interpreter, the full test suite passes
there in 11 s, and all four local ports answer.
...
Supported platforms (user decision 2026-08-31): **CCU3 (armv7l)** — explicitly including the
**official eQ-3 firmware**, not only OpenCCU — and **OpenCCU** (ex RaspberryMatic) for **aarch64**
and **x86_64**. No armv6l (Pi Zero/1), no i686 — RedMatic carried both and nobody used them.
Supporting the official firmware is what forces the custom armv7l runtime of H-39; everything else
runs stock binaries.
...
Field data, read-only from this house's CCU3 (`root@homematic-ccu3`, firmware 3.87.6, 2026-08-31):
armv7l, kernel 4.14.34, 4 cores, 970 MB RAM (809 used with RedMatic running); `/` is 976 MB (57 %
full, read-only) but **`/usr/local` is a 13 GB partition with 11.8 GB free** — a 50 MB runtime is
irrelevant there, only the CCU backup notices it (H-32); **no monit** (H-35 applies to OpenCCU only);
addons installed: `redmatic`, `patcher`; RedMatic's bundled Node is 14.16.0.
...
**Verified on the CCU3 (read-only, firmware 3.87.6, 2026-08-31)** — the details that decide how the
addon is written:

- `/etc/lighttpd/conf.d/cgi.conf` maps `.cgi` to `/bin/tclsh` **globally**, so the CGIs of §15.3 run
  with no lighttpd configuration of our own. Unlike RedMatic we need no proxy rule and therefore
  patch no lighttpd config at all.
- rc.d scripts are invoked by `run-parts` — `-a init` from `S55InitAddons` and `-a start` from
  `S98StartAddons` at boot, `-a stop` at shutdown. The script must accept **`init`** as well as
  `start|stop|restart|info|uninstall`, and its filename must be run-parts-safe (no dots).
- There is **no system-wide `update_addon`** (only RedMatic's own copy), **no `jq`**, **no `logread`**
  and **no `monit`** on CCU3. So: ship our own `update_addon`, keep the shell scripts jq-free (sh and
  tclsh only — RedMatic ships a jq binary precisely because the firmware has none), and treat monit
  as an OpenCCU-only extra (H-35).
- `syslogd` runs and writes `/var/log/messages` (rotating to `messages.0`); on this CCU it also
  forwards to a remote collector. Hence H-43.
...
### 15.2 The Node.js binary — bundle it, pinned to 24, self-built for armv7l

Verified 2026-08-31 against `nodejs.org/dist/index.json`, `unofficial-builds.nodejs.org`, the live
CCU3 (`root@homematic-ccu3`, read-only) and the **newest official firmware image** (ccu3-3.89.8.tgz,
July 2026, pulled from `ccu3-update.homematic.com` and its `rootfs.ext4` inspected locally):
...
- The last **LTS** line with an official armv7l build is **22 (Jod)**, EOL **2027-04-30**. Since
  armv7l needs our own build anyway (below), that ceiling stops mattering: **24 (Krypton)** runs to
  **2028-04-30** and **26** goes LTS on 2026-10-28 with EOL 2029-04-30. Pinning 22 would buy a
  runtime that dies first — hence H-29 pins **24**.
- Node still _supports building_ for armv7: `BUILDING.md` lists GNU/Linux armv7 as **Experimental,
  "downgraded as of Node.js 24"** — dropped from the release binaries, not from the source tree.
  Downstreams do build it: **Debian sid has `nodejs 24.19.0` for armhf** and — the one that matters —
  **Alpine edge has `nodejs 24.18.1-r1` for armv7**, linked against musl. The armv7l runtime is
  therefore a _packaging_ problem, not a compilation problem (H-39): that binary wants
  `/lib/ld-musl-armhf.so.1` and 16 shared libraries, and we ship all of them inside the addon with
  the ELF rewritten to look there and nowhere else.
- Caveat to validate, not to assume: that same table asks for **kernel ≥ 4.18**, and this CCU3 runs
  **4.14.34** (OQ-63). Building against older kernel headers is possible; whether V8/libuv touch a
  post-4.14 syscall at runtime is answered by running the binary, not by reading a table.
- unofficial-builds has **no armv7l build at any version** (its whole index: 0 hits); only x64 gets a
  `glibc-217` variant. There is no prebuilt escape hatch.
- hm2mqtt's `engines` (`^20.19 || ^22.12 || >=24`) covers 22 and 24 alike, so the pin is a free choice.
- **The CCU3 with official eQ-3 firmware cannot run any stock Node ≥ 18 armv7l binary** (OQ-55,
  answered): the running firmware 3.87.6 _and_ the current 3.89.8 both ship **glibc 2.27** and
  **libstdc++.so.6.0.24** (Buildroot 2019.08.2, gcc 8.3.0 — eQ-3 has not moved the toolchain in
  years). The official node binaries for 18, 20 and 22 armv7l all declare `GLIBC_2.28` in their
  version-need table and want `GLIBCXX_3.4.26` (gcc ≥ 9). Two independent floors, both above the
  firmware. RedMatic's 14.18.3 pin was not caution, it was the ceiling.
  The libstdc++ half is trivially fixable (ship the library and set `LD_LIBRARY_PATH`, as RedMatic
  does); the glibc half is not — hence H-39.
- **OpenCCU** (recent buildroot) is above both floors on all three arches and uses stock binaries.
- Two things the hardware run taught (2026-08-31), both now handled by `addon/build-runtime.sh`:
  **ICU data is not in the libraries.** Alpine links node against the system ICU whose
  `libicudata.so` is a 4 KB stub; the real data is `/usr/share/icu/<ver>/icudt78l.dat`, looked up at
  a path compiled into ICU. It ships inside the addon and the rc.d script exports `ICU_DATA`, or
  node does not start. (Alpine's default is `icu-data-en` — English only, so `Intl` formats
  everything as `en`; `icu-data-full` is the swap if locale-correct output ever matters, at +30 MB.)
  **The ELF patch is load-bearing, not cosmetic.** `node --test`, `child_process.fork` and anything
  else that re-executes `process.execPath` spawn the binary _directly_, so an unpatched interpreter
  fails every child instantly while the parent runs fine — the failure mode looks like broken tests,
  not a broken runtime. With the interpreter patched, forking and the whole suite work.
- Size on disk: **61 MB** (45 MB binary, ~15 MB libraries, ICU data), against 11.8 GB free on the
  CCU3's `/usr/local`. Irrelevant for space, relevant only for backups (H-32).
- Our runtime dependency tree (`binrpc`, `homematic-rega`, `homematic-xmlrpc`, `mqtt-interfaces-core`
  → `mqtt`, `yargs`) is **pure JavaScript** — no node-gyp, no per-arch module prebuilds, no QEMU for
  the addon build itself, no `prebuilt/<arch>` directory in git (RedMatic's sore point per its
  BUILD.md). One `npm ci --omit=dev` on the CI runner ships to all three arches; only `bin/node` differs.
...
the SPA renders a widget per option. The dump already exists: `node index.js --config-schema` prints
JSON Schema for all 43 options including each one's `x-env` name, and `addon/build.sh` writes it to
`www/config-schema.json` at build time — so the UI has no hand-maintained option list at all.
Generated, but not a dump of `--help`: each option additionally
...
| ID   | Decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| H-28 | **The addon bundles its own Node.js**, fetched at build time (RedMatic's `build_addon.sh` approach) — stock from nodejs.org for aarch64/x86_64, our own armv7l build (H-39/H-40) for the CCU3 — never a runtime dependency on another addon or on whatever OpenCCU happens to ship. Version pinned in one place: `package.json` `addon.node` (RedMatic used `engines.node` for this; we cannot, ours is a range).                                                                                                                                                                                                                                                                                                                                    |
| H-29 | **Node 24 (Krypton) for all three arches**, EOL 2028-04-30. aarch64 and x86_64 take the stock nodejs.org tarball; armv7l takes our own build (H-39), which is what frees the pin from the armv7l release ceiling (22, EOL 2027-04-30). One runtime version across arches — no mixed pins, one test matrix. Revisit at Node 26 LTS (2026-10-28) once the runtime build is routine.                                                                                                                                                                                                                                                                                                                                                                    |
...
| H-37 | **Local transport is the default when hm2mqtt runs on the CCU** (§15.4): binrpc on 32001/32000 for BidCos-RF/Wired, XML-RPC straight to hmipserver on 32010, VirtualDevices 39292, ReGa on 8183. Detection **probes the ports** (TCP connect to 32001/32000/32010/8183 on loopback) — it does **not** grep a lighttpd config the way node-red-contrib-ccu does, because that check is already broken on current firmware (§15.4). `--local`/`--no-local` overrides. Useful outside the addon too (Docker on the CCU with host networking).                                                                                                                                                                                                           |
...
| H-39 | **The armv7l runtime is repackaged from Alpine, not compiled.** Alpine edge builds current Node for armv7 against **musl** (`nodejs 24.18.1-r1`, verified 2026-08-31); we take that binary plus the musl loader and its `DT_NEEDED` closure and rewrite the ELF interpreter and RPATH to `/usr/local/addons/hm2mqtt/lib`. The CCU's glibc and libstdc++ stop being a variable — one package for official firmware and OpenCCU — and the build stays a two-minute step on a normal runner: no cross toolchain, no QEMU, nothing compiled. Implemented in `addon/build-runtime.sh`. Fallback if Alpine ever drops armv7 or moves Node out of that branch: cross-build musl-static ourselves (OQ-61). aarch64/x86_64 keep the stock nodejs.org tarball. |
| H-40 | **No separate runtime repo** — hm2mqtt's own addon build pulls the runtime in, into hm2mqtt's addon directory. Reviving `ccu-addon-nodejs` to publish runtime tarballs only paid off while the runtime needed a real cross-compile; with H-39 it is a download plus `patchelf`, so a second repo, a second release cadence and a second thing to keep alive would buy nothing. `ccu-addon-nodejs` stays retired.                                                                                                                                                                                                                                                                                                                                     |
| H-41 | **The addon carries the hm2mqtt version** — one number across npm, Docker and addon (`hm2mqtt-ccu-<arch>-<version>.tar.gz`, and the rc.d `info` prints the same `Version:`). Until someone has run it on real CCU hardware the addon assets get a **`-beta` suffix** (`3.4.0-beta`) while the npm and Docker artefacts of the same tag stay unsuffixed; the suffix is dropped in the release that declares it tested (the WebUI update check compares plain version strings, so it must not linger). The user's own CCU3 is **not** a test target for now (2026-08-31).                                                                                                                                                                              |
...
Detection: the reference greps `/etc/lighttpd/conf.d/proxy.conf` for `"port" => 32001` (the marker
for firmware ≥ 3.41, which introduced the 3xxxx ports) when the host is `127.*`/`localhost`. **That
check is dead on current firmware** — verified on this CCU3 (3.87.6, read-only, 2026-08-31):
`proxy.conf` is now a 38-byte `include "/var/etc/lighttpd_proxy.conf"` and the marker moved to
`proxy_normal.conf` / `proxy_httpsredirect.conf`, so `isLocal` never becomes true. The house's own
flow (`ccu-connection` node `host: localhost`, no `bcrfBinRpc`) is consequently running through the
lighttpd proxy on 2001/2010 today, not on the local ports — nobody noticed, which also says
something about how big the win is. We therefore **probe instead of grep**: a TCP connect to
32001/32000/32010/8183 on loopback, each interface independently, falling back to its proxy port.
All of them were confirmed listening on this CCU (`rfd:32001`, `hs485d:32000`, `ReGaHss:31999`
and `:8183`, hmipserver/`java:32010`, the last one on an IPv6 socket — use `127.0.0.1` and let
dual-stack handle it, or connect to `::1`). `--local`/`--no-local` overrides the probe (Docker on
the CCU with host networking is local too; a container with its own netns is not).
...
2. Runtime: `addon/build-runtime.sh <arch>` — stock nodejs.org tarball for aarch64/x86_64, Alpine musl
   repackage for armv7l (H-39); the result is `bin/node` (plus `lib/` on armv7l) and nothing else, per H-30.
...
6. `tar --owner=root --group=root -czf dist/hm2mqtt-ccu-<arch>-<version>.tar.gz *` plus `.sha256`, where
   `<version>` is the package version with the `-beta` suffix of H-41 while the addon is untested.
...
(the `github-release` job already creates it — the addon job attaches its assets with
`gh release upload`, before the notes are generated so H-42's table sees them). Plus a build-only
step in `ci.yml` on PRs so a broken `update_script`/`build.sh` is caught before a tag exists. All of
it on `ubuntu-latest` with no QEMU, because of H-31 — a full matrix is ~2 minutes.

The runtime step needs `docker` (so `apk` can resolve the armv7 closure) and `patchelf`, both a
one-line install on `ubuntu-latest`. The **`Addon runtime`** workflow builds it standalone —
`workflow_dispatch` for all three arches, automatically on PRs touching `addon/**` — and uploads the
tree as an artefact, so the packaging can be verified without cutting a release.

### 15.6 Steps

| #   | Step                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Effort |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| 1   | **done 2026-08-31** — glibc/Node floors checked on the live CCU3 and in the newest firmware image (§15.2, OQ-55): official firmware is glibc 2.27 / libstdc++ 6.0.24, stock Node ≥ 18 armv7l is out.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | —      |
| 1b  | **done 2026-08-31** — `addon/build-runtime.sh`: Alpine musl repackage for armv7l (apk resolves the foreign-arch closure in a container, patchelf points interpreter and RPATH into the addon), stock tarballs for aarch64/x86_64, self-check that nothing references anything outside the prefix; exercised by the `Addon runtime` workflow. Open: run it on a real CCU3 (kernel 4.14, OQ-63).                                                                                                                                                                                                                                                                                           | —      |
| 2   | **done 2026-08-31** — `addon/files/`: `update_script` (config survives updates, three platform links, nothing else touched), `rc.d/hm2mqtt` with the `init` verb and pid-file service control, `hm2mqtt.cfg`, `bin/update_addon` **as a tcl script** (the firmware ships none and RedMatic's is a compiled binary — hm_addons.cfg is a Tcl dict, so tclsh edits it; verified against this CCU's real file: idempotent add, clean remove, other addons untouched), `etc/default.env`, `etc/monit.cfg`, `www/` with session-checked `settings.cgi`, `update_check.cgi` and a placeholder page until step 5.                                                                                | —      |
| 3   | **done 2026-08-31** — `addon/build.sh <arch> [--beta]`: runtime + addon files + `npm pack`ed hm2mqtt with production dependencies + `--config-schema` dump for the UI + `versions` + `.nobackup` markers → `dist/hm2mqtt-ccu-<arch>-<version>[-beta].tar.gz`. Verified locally for x86_64 (145 MB installed, 45 MB package). CI builds all three arches; a real install test on hardware is still open (OQ-62).                                                                                                                                                                                                                                                                          | —      |
| 4   | **done 2026-08-31** — CGIs: `getconfig` (passwords never leave the CCU — the browser sees a placeholder), `setconfig` (validates every line against `HM2MQTT_KEY=value` and restores a password left at the placeholder), `service` (start/stop/restart/status incl. pid, RSS, uptime, versions), `log`, `api` (whitelisted bridge to `scripts/addon-api.js`), all session-checked against ReGa. Paths derive from `info script`, so the tree is relocatable and testable: `addon/test/cgi-test.sh` runs all of it without a CCU (tclrega stubbed) — 16 assertions, in CI.                                                                                                               | —      |
| 5   | **done 2026-08-31** — Svelte 5 UI in `addon/ui/`, built by Vite into **one 65 kB `index.html`** (24 kB gzipped, everything inlined — no asset paths on the CCU). Renders from `config-schema.json`, so all 43 options are present by construction; groups and widgets come from `addon/ui/src/options.js`; service control, status line, log view and a de/en switch. Interactive helpers via `scripts/addon-api.js`: find the CCU, probe the interfaces, test the broker, preview an item template — all verified against the live CCU (probe found 4 interfaces, preview rendered from 886 real channels). `test/addon-ui.test.js` fails when an option has no place in the UI (H-38). | —      |
| 6   | **done 2026-08-31** — `addon` job in `release.yml` (matrix armv7l/aarch64/x86_64, runs the CGI tests, builds with `--beta`), its artefacts collected by the `github-release` job and attached to the release; README section "On the CCU itself (addon package)" with the platform table.                                                                                                                                                                                                                                                                                                                                                                                                | —      |
| 7   | **done 2026-08-31** — local mode (H-37): `local` flag through `interfaceConfig()`/`probeInterfaces()`, `regaPort()` (8183), `isLocalHost()`/`detectLocal()` probing the process ports, `--local`/`--no-local`, loopback callback servers, 7 unit tests. All four ports confirmed open on the CCU3.                                                                                                                                                                                                                                                                                                                                                                                       | —      |
| 8   | **done 2026-08-31** — Installation table in `.github/release-notes.js` (H-42), rendered from the assets in `dist/` so it never names a file the release does not carry, with the `-beta` note of H-41.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | —      |

### Open questions

| ID    | Question                                                                                                                                                                                                                                                                                                                                                                                                                                        | Proposal                                                                                                                                                                                                                                                                            |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OQ-55 | **answered 2026-08-31** — Does an official Node ≥ 20 `linux-armv7l` binary run on the **CCU3 firmware**? **No.** Running firmware 3.87.6 and the newest 3.89.8 both ship glibc 2.27 / libstdc++ 6.0.24 (Buildroot 2019.08.2, gcc 8.3.0); the binaries need `GLIBC_2.28` and `GLIBCXX_3.4.26`.                                                                                                                                                   | Official firmware stays supported (user decision) → build our own armv7l runtime, H-39.                                                                                                                                                                                             |
| OQ-56 | **answered 2026-08-31** — where does the broker come from on a CCU-only setup? Not from the platform: OpenCCU's buildroot tree has no mosquitto, no CCU-Jack, no broker of any kind (35 packages, checked via the GitHub API), and the CCU3 firmware has none either.                                                                                                                                                                           | Document two paths — point at an existing broker on the network (the normal case), or install the Mosquitto addon — and do not bundle one. The addon's first-run UI asks for the broker URL and tests it (§15.3), which is where this question actually gets answered for the user. |
| OQ-57 | **answered 2026-08-31** — is there an OpenCCU addon index to register with? **No.** The `OpenCCU` org has five repos (platform, device firmware, ReGaHss tests, xmlrpcpp, community files) and no store or index; third-party addons are plain GitHub releases, found through the forum and the project wiki.                                                                                                                                   | Nothing to build for it. Announce in the Homematic forum and link from the README; keep the GitHub release the single source of the package.                                                                                                                                        |
| OQ-58 | **decided 2026-08-31** — HA discovery and ReGa poll defaults on a CCU3 (970 MB RAM shared with ReGa/rfd, ~775 channels).                                                                                                                                                                                                                                                                                                                        | Same defaults as everywhere else, but written into `etc/default.env` so an addon profile can diverge without a code change. Measure RSS on hardware during the first install test; if discovery is too heavy there, flip it in `default.env` only.                                  |
| OQ-59 | **decided 2026-08-31** — does the addon replace `--install`/`--uninstall`, or does `lib/install.js` grow a CCU mode?                                                                                                                                                                                                                                                                                                                            | Addon only. There is no systemd on the CCU; the rc.d script is the unit, and the addon's `update_script` is the installer. `--install` stays a systemd-only feature and the UI hides it (H-38).                                                                                     |
| OQ-60 | **answered 2026-08-31** — the `proxy.conf` marker check is broken on firmware ≥ 3.8x (the file is now an `include`); the 3xxxx ports themselves are all present and listening.                                                                                                                                                                                                                                                                  | Probe the ports instead of grepping (H-37); still worth a look on an OpenCCU image before release.                                                                                                                                                                                  |
| OQ-61 | **answered 2026-08-31** — how to get a Node ≥ 24 armv7l binary that runs on glibc 2.27: repackage Alpine's musl build (H-39). Remaining risk: Alpine edge is a rolling branch and its armv7 support is not eternal.                                                                                                                                                                                                                             | Pin `NODE_MAJOR` and fail the build when the branch drifts to another major (the script does). If armv7 disappears there: cross-build musl-static with musl-cross-make, ~1–2 h per Node version in CI — the route this replaced, kept documented for that day.                      |
| OQ-63 | **answered on hardware 2026-08-31** — does Alpine's musl Node run on the CCU3's 4.14.34 kernel? **Yes.** Node 24.18.1 started, reported `arm/linux`, and hm2mqtt's **full test suite passed there: 69 tests, 21 suites, 0 failures, 11 s** (4 cores, 970 MB RAM). OpenSSL 3.5.8 with 62 TLS ciphers, crypto, loopback networking and Intl all work. The kernel ≥ 4.18 line in `BUILDING.md` is the glibc builds' floor and does not apply here. | Closed. The test ran from `/usr/local/tmp` and was removed afterwards; nothing was installed.                                                                                                                                                                                       |
| OQ-62 | **Who tests the addon on real hardware** — everything short of the install itself is now verified on the CCU3 (runtime, patched ELF, child processes, ports, `versions` sourcing, 69 tests), but no one has yet run `Zusatzsoftware → hm2mqtt` end to end. The `-beta` suffix (H-41) comes off when a CCU3 with official firmware and an OpenCCU install are confirmed.                                                                         | Announce in the Homematic forum when the first release carries the packages; until then the notes and README say beta.                                                                                                                                                              |

---

## 16. openccu-lite — names without ReGaHSS (2026-09-06)

[openccu-lite](https://github.com/hobbyquaker/openccu-lite) is a CCU firmware **without ReGaHSS**
(D-1): nothing listens on 8181/8183, HM-Script is never interpreted, and there is no ReGa DOM.
What it has instead is `occulited`'s **metadata API** (D-16, `/api/meta/v1/`), a JSON store of
names and enum trees with an event stream, and a `tclrega.so` shim that answers exactly the
session check every addon settings page makes. The port follows the project's porting kit
(`docs/porting-from-rega.md`, `docs/PORTING-PROMPT.md`, fixtures in `fixtures/store/`), whose
worked example is this repository.

### Decisions

| ID   | Decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| H-46 | **A second names provider, not a fork of the first.** `lib/meta.js` (`MetaSync`) has the same public surface as `lib/rega.js` (`RegaSync`) — `channelName()`, `channelAddress()`, `rooms()`, `functions()`, `syncNames()`, the `names`/`polled`/`error` events, `channelNames` — so index.js and everything below it never learns which box it is talking to and no topic or payload changes. `sysvars`/`programs` stay empty, `hasVariable()`/`hasProgram()` are false. Both providers share `lib/names.js` (name file, reverse index, address lookup); `label` (`rega` / `occulite`) is what the shared log lines print.                                                                                                                                                                                          |
| H-47 | **Detection is one unauthenticated call, at runtime.** `GET /api/meta/v1/version` at start (before the provider is built): a version document means openccu-lite, a 404 or HTML means ReGa. No hostname, port or `/VERSION` heuristics, no new mandatory option — the same configuration works on both kinds of box, which a restored backup needs. A box that answers _nothing_ is probed again every 60 s (`REDETECT_MS`), because a box that boots after us must not need a restart of the service; a box that answered something is never probed again. `--no-rega` still means "addresses only" on both.                                                                                                                                                                                                       |
| H-48 | **Rooms and functions are the names of the nodes an object is in, ancestors not added.** The box keeps trees (`room/eg/wohnzimmer`) and membership is inherited upwards, but `hm.rooms` must stay what a CCU produced: `['Wohnzimmer']`, not `['Erdgeschoss', 'Wohnzimmer']` — otherwise `${room}` in a topic template changes meaning and `hm.room` (the singular, "exactly one room") disappears for every channel. Enums other than `room` and `function` (`floor`, user-created) are ignored: there is no field for them.                                                                                                                                                                                                                                                                                       |
| H-49 | **What the box cannot do is one log line, not an error loop.** `--rega-poll-interval`, `--rega-poll-trigger`, `--publish-cache` and the sysvar/program topic templates stay accepted and do nothing there; at start one `info` line names them, and `loadCache()` is not even attempted (there is no `getValues`). One configuration serves both kinds of box.                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| H-50 | **The credential is the box's, not a new secret.** On the box (the addon case) the read-only local token from `/usr/local/etc/occulite/local-token`; anywhere else `--meta-token` (`secret: true`, like the CCU and broker passwords), an API token created on the box. `--meta-url` overrides the base url for a proxy or a non-standard port; both are `NOT_APPLICABLE` in the addon UI (H-38) because on the box neither is a decision. A 401 logs **once**, keeps whatever names are cached and retries on the next stream reconnect and every `syncNames()`; `connected` is `1` while it lasts, which is what H-5 means by degraded.                                                                                                                                                                           |
| H-51 | **Snapshot first, then the event stream** (`/events/sse`, reconnect with `?since=<revision>`, exponential backoff to 5 min, socket timeout 95 s against the box's 30 s heartbeat). `object.updated`/`object.deleted` are applied incrementally; every `enum.*`, `node.*`, `import`, `resync` and every revision gap re-reads the snapshot, coalesced — the paths of members and the names of rooms change under those, and a snapshot is the cheapest correct answer. The store is persisted as `meta.json` next to `rega.json` (the raw document, so a restart resumes at its revision), and the ReGa cache is left alone so a box swap does not destroy the other one's names. No new dependency: `node:http`, because Node 20 has no `EventSource` and it also gives us the socket timeout and `--ccu-insecure`. |
| H-52 | **The addon package installs unchanged**, session check included (`rega_script "Write(system.GetSessionVarStr(...))"`, answered by the shim). The only addition is where the pid file goes: `/run/addon-hm2mqtt/` when that directory exists (openccu-lite runs addons in generated systemd units and may confine them to their own user, where `/var/run` is root's), `/var/run/hm2mqtt.pid` otherwise — decided the same way in `rc.d/hm2mqtt` and `www/lib/common.tcl`.                                                                                                                                                                                                                                                                                                                                          |

### Tests

- `test/meta.test.js` — the conformance corpus in `test/fixtures/meta/` (copied from openccu-lite's
  `fixtures/store/`, valid and invalid documents) and a fake `/api/meta/v1` server: detection
  (version document, a CCU's HTML, nothing listening), snapshot → names → event → `names` event,
  deletion, room rename, revision gap, import, the 401 path (one warning, recovery), an unreachable
  box, and reconnect with `?since`.
- `test/occulite.test.js` — the real thing, opt-in (`HM2MQTT_OCCULITE=<occulited binary>
npm run test:occulite`): `occulited` with its own state directory, the admin created through
  `POST /api/auth/v1/setup`, a document imported through `PUT /api/meta/v1/import`, hm-simulator on
  the interfaces and aedes as the broker — the whole adapter publishing `hmtest/status/<name from
the box>/PRESS_SHORT` with `hm.rooms`/`hm.functions` from the box's enums, a rename on the box
  moving the topic without a restart, the local token's `403` on a write, and `meta.json` on disk
  after SIGTERM.
- The ReGa tests are untouched and green (`test/rega.test.js`, `test/e2e.test.js`).

### Open

| ID    | Question                                                                                                                                                                                                                                                                                                                                                                    |
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OQ-64 | Nothing has been run on a real openccu-lite box yet — only against `occulited` built from source on a laptop. The addon package's install on that firmware (confined or not, the pid file path of H-52) is untested, as is the local token file at `/usr/local/etc/occulite/local-token` (the test reads it from occulited's state directory, which is that path on a box). |
| OQ-65 | openccu-lite's interface processes listen on loopback only (D-29), so an off-box hm2mqtt needs the box's XML-RPC proxy; the addon package on the box is the intended deployment. Not exercised here.                                                                                                                                                                        |
| OQ-66 | The metadata API can also publish the store to MQTT itself (`occulited.json` → `mqtt`). Two writers of names on one broker is a configuration users will try; nothing in hm2mqtt reads or collides with `occulite/meta/#`, but the overlap is worth a note in the box's documentation.                                                                                      |

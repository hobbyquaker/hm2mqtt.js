...
| OQ-63 | Node's `BUILDING.md` asks for **kernel ≥ 4.18** on armv7; the CCU3 runs **4.14.34**. Is that a real runtime dependency (a syscall V8/libuv needs) or just the build baseline? Alpine's build makes no promise here either.                                                                    | Answer empirically with the first packaged runtime: run `node -e` and the hm2mqtt test suite on a CCU3. If a syscall is genuinely missing, the ladder is Node 22 → 20 for armv7l only, at the cost of a mixed pin.                                             |
